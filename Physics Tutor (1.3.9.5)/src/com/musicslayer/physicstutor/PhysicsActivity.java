...
public class PhysicsActivity extends Activity {
    ...
    static int surveyChoice=0, option[]={1,1,1,1}, selection_g=0, answerChoice=0, defaultLength=0, defaultTime=0, defaultVelocity=0, defaultAngle=0, defaultMass=0, defaultForce=0, defaultEnergy=0, defaultFrequency=0, defaultAcceleration=0;
    ...
    public void savePrefs() {
        ...
        editor.putInt("survey", surveyChoice);
    ...
    public void loadPrefs() {
    	SharedPreferences preferences = getSharedPreferences("data", MODE_PRIVATE);
        if (!"PROJECTILE".equals(type) && !"VECTORS".equals(type) && !"INCLINE_SIMPLE".equals(type) && !"INCLINE".equals(type) && !"SPRING_SIMPLE".equals(type) && !"SPRING".equals(type))
        {
            scale=preferences.getFloat("scale", scale);
            adTimer.setTime(preferences.getLong("timer", adTimer.getTime()));
		    type=preferences.getString("type", "?");
            surveyChoice=preferences.getInt("survey", surveyChoice);
